...
#define ARQUIVOSTATE    "/opt/firebird/uidstate"
     ...
     uuid_node_t node);
     static void format_uuid_v1(uuid_tt * uuid, unsigned16 clockseq,
     uuid_time_t timestamp, uuid_node_t node);
     static void format_uuid_v3(uuid_tt * uuid, unsigned char hash[16]);
	...
	FILE *arquivo;
#define LOGA        arquivo=fopen("/tmp/bostadocaralho2","w+");
       int f;
...
//	LOGA;
     ...
     /* format_uuid_v1 -- make a UUID from the timestamp, clockseq,
                          and node ID */
     void format_uuid_v1(uuid_tt * uuid, unsigned16 clock_seq, uuid_time_t
     ...
     void uuid_create_from_name(
       ...
       uuid_tt net_nsid;      /* context UUID in network byte order */
       ...
       MD5Init(&c);
       MD5Update(&c, &net_nsid, sizeof(uuid_tt));
     ...
     */
     void format_uuid_v3(uuid_tt * uuid, unsigned char hash[16]) {
         /* Construct a version 3 uuid with the (pseudo-)random number
          * plus a few constants. */

         memcpy(uuid, hash, sizeof(uuid_tt));
